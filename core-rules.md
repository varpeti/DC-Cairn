...
Based on the situation each die can have multiple Advantage or Disadvantage.
...
- Held Action (X AP): You describe an [Action](#actions) and the condition(s), and when the condition(s) are met, the [Action](#actions) happens.
  ...
  - Now: Use the AP for the Action(s). Decide the modifiers, Object Dice, Advantage(s) and Disadvantage(s) with the Warden.
  - When then Action happens: Use the other resources your Action requires, then roll for the Action.

## Scars

If damage to a PC would reduce their HP to 0 or under, look up the result on the [Scars Table](#scars-table), based on _how much HP the PC had before the last attack_
